...
// Find max items
const maxItems = ui.sort((x, y) => y.evaluate.length - x.evaluate.length)[0].evaluate.length;
    ...
    for (let i = 0; i < maxUsers; i++) {
        ...
        for (let j = i; j < maxUsers; j++) {
            if (j === i) {
                ...
                simCos[i][j] = Math.round(100 * (sumRue / (Math.sqrt(sumPowRu) * Math.sqrt(sumPowRe)))) / 100;
            }
        }
    }
    console.log('----SimCos-----')
